...
function showTreemap(data) {
  ...
  tree.children  = data.map((d) => {
    return {
      ...
      label: d.caption,
      img: () => d.thumbnail_src,
      url: "http://www.instagram.com/p/" + d.code,
      value: d.likes.count,
      labelValue: fmt(d.likes.count),
      node: d
    }
  })
...
function refresh(user) {
  ...
  d3.json(query, 
    ...
    if (!data.user.media.nodes || (data.user.media.nodes.length===0&&photos.length===0)) {
      alert("This user is private or doesn't have photos");
      return;
    }
    treeMap.data = data;
    photos= photos.concat(data.user.media.nodes);
...
function refreshTreemap() {
  utils.setGetParameter("user", treeMap.data.user.username);
  d3.select("#totalPhotos")
    .text(photos.length);
  d3.select("#user")
    .text("@" + treeMap.data.user.username);
  ...
  showTreemap( d3.select("#chkBest").property("checked") ? 
    photos.sort((a,b) => 
      d3.descending(a.likes.count, b.likes.count))
