...
function TreeMap(htmlID) {
    ...
    var ZOOM_TRANSITION_DURATION = 750;
   ...
   //Alex grande answer in http://stackoverflow.com/questions/5999998/how-can-i-check-if-a-javascript-variable-is-function-type
    ...
    function treeTraverse(fn) {
        function treeTraverseHelper(node) {
            var filteredChildren = node.children!== undefined ? filter(node.children) : [];
            if (filteredChildren) {
    ...
    self.update = function (root, _currentNode) {
        ...
        //Draw, then animate
        var node = updateHelper(currentNode);
        updateImg(node);
        node.transition()
            .duration(750)
            .call(position)
    ...
    function accumulate(d) {
        ...
        if (filteredChildren) {
            ...
            var accum = filteredChildren.reduce(function(p, v) { return p + accumulate(v); }, 0);
            if (accumulateValue) {
                d[value] = accum;
            }
        }
    ...
    self.jumpIntoRandom = function() {
        ...
        var randI = Math.floor(Math.random() * options.length);
        var randomNode = options[randI];
    ...
    var jumpIntoHelper = function (d) {
        ...
        var t2 = g2.transition().duration(ZOOM_TRANSITION_DURATION);
    ...
    var position = function (sel) {
        ...
        sel
            .style("left", function(d) { return x(d.x) + padding(d) + "px"; })
            .style("top", function(d) { return y(d.y) + padding(d)  + "px"; })
            .style("width", function(d) { return  x(d.x + Math.max(0, d.dx )) - x(d.x) -  padding(d) + "px"; })
            .style("height", function(d) { return y(d.y + Math.max(0, d.dy )) - y(d.y) -  padding(d) + "px"; });
        sel.select(".nodeBG")
        ...
        if (self.zoomable) {
            sel.select(".nodeText")
                ...
                .style("top", function(d) { return  (y(d.y + d.dy) - y(d.y) - TEXT_HEIGHT ) / 2  + "px"; });
        } else {
            sel.select(".nodeText")
                .style("left", function(d) { return 0 + "px"; })
                .style("top", function(d) { return  (y(d.y + d.dy) - y(d.y) - TEXT_HEIGHT)  + "px"; });
        }
